...
import {
  Loader2, ArrowRight, Sparkles, TrendingUp,
...
function computeStats(currentOrders: Order[], prevOrders: Order[]) {
  ...
  for (const order of currentOrders) {
    ...
    dayRevMap[day] += Number(order.total_price);
  }
  const revenueByDay = DAY_NAMES_HE.map((name, i) => ({
    day: name,
    revenue: Math.round(dayRevMap[i]),
  }));
  ...
  const revenueByHour = Object.entries(hourMap).map(([h, orders]) => ({
    hour: `${h}:00`,
    orders,
  }));
...
function WeeklyPromoCard({
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      const badgeText = promo.discount;
      ctx.font = 'bold 44px Arial';
      const badgeW = ctx.measureText(badgeText).width + 60;
      const badgeX = 40;
      const badgeY = size - 70;
      ctx.beginPath();
      ctx.roundRect(badgeX, badgeY - 50, badgeW, 60, 30);
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      ctx.textAlign = 'left';
      ctx.fillText(badgeText, badgeX + 30, badgeY - 5);
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {promo.targetItems.map((item, i) => (
                <span
                  key={i}
                  className="bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 text-xs px-2.5 py-1 rounded-full font-medium"
                >
                  {item}
                </span>
              ))}
          ...
          {promoImage ? (
            <div className="space-y-3">
              <div className="relative rounded-xl overflow-hidden shadow-md">
                <img
                  src={`data:image/png;base64,${promoImage}`}
                  alt="תמונת מבצע"
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-5 text-right">
                  ...
                  <h3 className="text-white text-2xl font-black leading-tight mb-2 drop-shadow-lg">
                    {promo.title}
                  </h3>
...
const AIInsightsPage: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // New format: structured JSON with recommendations array
      if (Array.isArray(data?.recommendations)) {
        const recs = data.recommendations;
        const promo = data?.weeklyPromotion && typeof data.weeklyPromotion === 'object'
          ? {
              ...data.weeklyPromotion,
              targetItems: Array.isArray(data.weeklyPromotion.targetItems)
                ? data.weeklyPromotion.targetItems
                : [],
            }
          : null;
        setInsights({ recommendations: recs, weeklyPromotion: promo });
      } else if (typeof data?.recommendations === 'string') {
        // Legacy format: plain markdown string — show as-is until function is redeployed
      ...
      <header className="border-b border-border bg-card sticky top-0 z-10">
        <div className="container flex items-center gap-4 h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-foreground leading-tight">המלצות AI</h1>
              <p className="text-xs text-muted-foreground leading-tight">ניתוח מכירות שבועי + מבצע</p>
            </div>
          </div>
        </div>
      </header>
                      ...
                      <CardContent className="px-4 pb-4">
                        {stats.top5Products[0] ? (
                          <>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">{stats.top5Products[0].units} יחידות · ₪{stats.top5Products[0].revenue}</p>
                          </>
                        ) : (
            ...
            <div className="flex justify-center">
              ...
              >
                {generating ? (
                  ...
                  <>
                    <Sparkles className="w-4 h-4" />
                    {insights ? 'רענן המלצות' : 'צור המלצות AI + מבצע'}
                  </>
                )}
              </Button>
            </div>
              ...
              <Card className="bg-white dark:bg-card border-purple-200 dark:border-purple-800">
                <CardHeader className="pb-2">
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                    ⚠️ יש לפרוס מחדש את ה-function כדי לקבל את הפורמט המשודרג עם מבצע השבוע
                  </p>
                </CardHeader>
                <CardContent>
